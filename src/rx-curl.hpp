		...
		worker = observable<>::create<CURLMsg *>([this](subscriber<CURLMsg *> out)
				...
				out.on_completed(); }) |
				 subscribe_on(thread) |
						 ...
						 { std::cout << "RxCurl worker exit" << std::endl; }) |
				 publish() |
				 connect_forever();
	}
	...
	observable<HttpResponse> create(HttpRequest request)
	{
		return observable<>::create<HttpResponse>([=](subscriber<HttpResponse> &out)
		{
			...
			state->worker
				...
				{
					auto rs = wrs.lock();
					bool helper_bool = !!rs && !!message && message->easy_handle == rs->curl && message->msg == CURLMSG_DONE;
					if(helper_bool)
					{
						std::cout << "helper_bool: true" << std::endl;
					}
					else
					{
						std::cout << "helper_bool: false" << std::endl;
					}
					return helper_bool;
				})
				.take(1)
				.tap([&,wrs](CURLMsg* message)
				{
					...
					if (message->data.result != CURLE_OK)
					{
						...
						std::cout << "RxCurl request fail: " << httpStatus << " - " << rs->error << std::endl;
						observable<>::error<std::string>(HttpException(rs)).subscribe(chunkout);
						return;
					}
					else if (httpStatus > 499)
					{
						std::cout << "RxCurl request http fail: " << httpStatus << " - " << rs->error << std::endl;
						observable<>::error<std::string>(HttpException(rs)).subscribe(chunkout);
						return;
					}

					std::cout << "RxCurl request complete: " << httpStatus << " - " << rs->error << std::endl;
